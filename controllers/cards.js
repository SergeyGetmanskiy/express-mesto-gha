  ...
  Card.create({ name, link, owner: req.user._id })
    ...
    .catch((e) => {
      if (e.name === 'ValidationError') {
        return res.status(ERR_BAD_REQUEST).send({ message: `Переданы некорректные данные при создании карточки. ${e.message}` });
      }
...
module.exports.likeCard = (req, res) => Card.findByIdAndUpdate(
  ...
  .catch((e) => {
    ...
    if (e.name === 'DocumentNotFoundError') {
      return res.status(ERR_NOT_FOUND).send({ message: 'Передан не существующий _id карточки.' });
    }
    return res.status(ERR_DEFAULT).send({ message: e.message });
  });
